...
- **`OnnxEmbeddingModel`** — ONNX Runtime implementation backed by
  BGE Small En v1.5, delivering dense embeddings suitable for semantic search
  and retrieval tasks.
...
- **`ModelCatalog`** — allowlist provider that gates model use behind
  ...
  models: `bge-small-en-v1.5` (validated), `bge-m3-v1.0` (registered,
  not yet validated — throws `UnsupportedError`).
