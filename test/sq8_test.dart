...
void main() {
  group('SQ8 quantise / dequantise', () {
    ...
    // ── Empty-vector assertions (debug mode) ──────────────────────────────

    test('quantise assert fires for empty vector', () {
      expect(() => quantise(Float32List(0)), throwsA(isA<AssertionError>()));
    });

    test('dequantise assert fires for empty vector', () {
      expect(() => dequantise(Uint8List(0)), throwsA(isA<AssertionError>()));
    });
  });
}
